...
class TestSyncCSV:
    ...
    def test_initialize(self, sync_csv, mock_logger):
        with patch.object(sync_csv, '_load_csv_mapping') as mock_load_csv_mapping, \
                patch.object(sync_csv, '_set_comparison_properties') as mock_set_comparison_properties:
            sync_csv.initialize()
    ...
    def test_load_csv_mapping(self, sync_csv, mock_logger):
        with patch('sync.sync_strategy.initialize_or_get_user_config_file', return_value='path/to/mapping.json'), \
                patch('sync.sync_strategy.CSVMapping') as mock_csv_mapping:
            sync_csv._load_csv_mapping()
            mock_csv_mapping.assert_called_once_with(mapping_file_path='path/to/mapping.json')
    ...
    def test_get_source_users(self, sync_csv, mock_logger):
        with patch.object(sync_csv, '_get_csv_data_path', 
                          return_value='path/to/csv_file.csv') as mock_get_csv_data_path, \
                patch.object(sync_csv, '_parse_csv_file', 
                             return_value=[{'field1': 'value1', 'field2': 'value2'}]) as mock_parse_csv_file, \
                patch.object(sync_csv, '_validate_csv_users', 
                             return_value=['validated_user']) as mock_validate_csv_users:
    ...
    def test_parse_csv_file_with_disabled_user(self, sync_csv):
        ...
        with patch('builtins.open', new_callable=mock_open, read_data=csv_content):
            sync_csv.mapping = {"Extension": {"New": {"field1": "header1", "field2": "header2", "Enabled": "Enabled"}}}
            result = sync_csv._parse_csv_file("path/to/csv_file.csv")
            expected = [
                {"field1": "value1", "field2": "value2", "Enabled": "0", "HotdeskingAssignment": ""},
    ...
    def test_validate_csv_users(self, sync_csv):
        ...
        with patch('sync.sync_strategy.TypeAdapter.validate_python', return_value=['validated_user']) as mock_validate:
