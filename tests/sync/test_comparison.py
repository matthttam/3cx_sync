...
class TestUserChangeDetail:
    def test_user_change_detail_initialization(self):
        ...
        user_change_detail = UserChangeDetail(
            field_changes=field_changes, user_to_update=user_to_update
        )
    ...
    def test_user_change_detail_id_property(self):
        user_to_update = User(Id=123, Number="456")
        user_change_detail = UserChangeDetail(
            field_changes={}, user_to_update=user_to_update
        )
...
class TestUserComparer:
    ...
    def test_init(self, custom_sync_source):
        ...
        comparer = UserComparer(
            tcx_user_list=tcx_user_list, sync_source=custom_sync_source
        )
    ...
    def test_get_user_change_details(self, custom_sync_source):
        ...
        assert user_change_details[0].field_changes == {
            "FirstName": FieldChange(old="Old Name", new="New Name")
        }
    ...
    def test_compare_user(self, custom_sync_source):
        ...
        user_change_detail = comparer.compare_user(
            tcx_user_list[0], source_user, update_fields
        )
