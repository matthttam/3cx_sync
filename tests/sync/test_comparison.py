...
class TestUserChangeDetail:
    ...
    def test_is_enabling_false(self):
        field_changes = {
            "Enabled": FieldChange(old=True, new=False),
            "field2": FieldChange(old="old2", new="new2"),
        }
        user_change_detail = UserChangeDetail(field_changes=field_changes)

        assert user_change_detail.is_enabling == False

    def test_is_enabling_true(self):
        field_changes = {
            "Enabled": FieldChange(old=False, new=True),
            "field2": FieldChange(old="old2", new="new2"),
        }
        user_change_detail = UserChangeDetail(field_changes=field_changes)

        assert user_change_detail.is_enabling == True
...
class TestUserComparer:
    ...
    def test_get_user_change_details(self, custom_sync_source):
        tcx_user_list = [
            User(Id=1, Number="100", FirstName="Old Name"),
            User(Id=2, Number="101", FirstName="Old Name"),
        ]
        comparer = UserComparer(tcx_user_list=tcx_user_list, sync_source=custom_sync_source)
    ...
    def test_compare_user_no_changes(self, custom_sync_source):
        # Get the same list of users we are going to compare against so there are no changes
        tcx_user_list = custom_sync_source.get_source_users()
        comparer = UserComparer(tcx_user_list=tcx_user_list, sync_source=custom_sync_source)
        update_fields = ["FirstName"]
        user_change_detail = comparer.compare_user(tcx_user_list[0], tcx_user_list[0], update_fields)
        assert user_change_detail.field_changes == {}
